...
<script>
    layui.use(['form', 'table'], function () {
        ...
        table.on('toolbar(currentTableFilter)', function (obj) {
            if (obj.event === 'add') {  // 监听添加操作
            ...
            }else if (obj.event === 'editque') {  // 监听设计问卷操作
            ...
            }else if(obj.event === 'preview'){ // 监听预览问卷操作
                var checkStatus = table.checkStatus('currentTableId')
                    , data = checkStatus.data;
                var arr=[];
                for(index in data){
                ...
                if(arr.length !=1){
                    layer.msg("请选择一行问卷预览",{time:1000});
                    return;
                }
                window.open("preview/"+arr[0]);
            }else if(obj.event === 'publish'){ // 监听发布问卷操作
            ...
            }else if(obj.event === 'query_detail'){ //监听问卷详情操作
                var checkStatus = table.checkStatus('currentTableId')
                    , data = checkStatus.data;
                var arr=[];
                for(index in data){
